...
import { DEFAULT_MODEL } from "../llm/index.ts";
...
export interface AgentSelf {
  ...
  tools: AgentSelfTool[];
  /** The model this agent is configured to think with — i.e. what a NEW
   *  thread resolves (OLLE_MODEL override → `thinking-model` memory → host
   *  default). This is the authoritative answer to "what model am I?",
   *  distinct from `recentlyPricedModels` (ledger spend history). Note: a
   *  long-lived thread that started before a switch may still be frozen on
   *  its older model — this field is the current setting, not necessarily
   *  every in-flight thread's model. */
  thinkingModel: string;
  /** True when no explicit choice is set and `thinkingModel` is the host
   *  default (DEFAULT_MODEL). */
  thinkingModelIsDefault: boolean;
  /** The agent's configured reasoning effort, or "off" when thinking is
   *  disabled (the default). Mirrors the per-thread `effort`. */
...
export function agentSelf(store: Store, agentId: string): AgentSelf | null {
  ...
  // The model/effort the agent is actually configured to think with — same
  // resolution the daemon uses to pick a new thread's model. This is the
  // truthful "what model am I?" answer; recentlyPricedModels is spend history.
  const chosenModel = resolveBootModel(store, agentId);
  const thinkingModel = chosenModel ?? DEFAULT_MODEL;
  // Clamp the effort against the SAME model the thread will run, exactly as
  // the daemon does at loop start — otherwise a Sonnet/Haiku agent with a
  // stored `max` would report "max" while actually running clamped-down (or
  // off). Without the model arg this falls back to DEFAULT_MODEL (Opus, which
  // supports every level) and overstates the effort.

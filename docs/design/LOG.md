...
## 2026-07-11 — CLI-brain fallback: run turns through a logged-in `claude`/`codex`

A host with no API key but a logged-in coding-agent CLI can now back the chat loop: OLL-E spawns that CLI as the LLM, and its own tools reach the harness over MCP (`olle mcp-bridge` proxies `tools.list`/`tools.call` into the daemon through the same scope gate + audit events the API loop uses). The detection ladder is: secret-file API key → env-var API key (imported to the file) → `claude` CLI probe → `codex` CLI probe → disabled.

Why the subprocess and not the credentials: reading `~/.claude/.credentials.json` or minting our own OAuth against Anthropic's consumer endpoints is ToS-prohibited and gets the *user* banned. Invoking the official binary is the explicitly sanctioned path — the CLI is what the subscription is licensed for. So CLI mode is a first-class fallback brain, accepting the tradeoffs it forces: the harness owns its inner loop, so OLL-E's SOUL/cache economy and exact API token accounting don't apply — usage is whatever the CLI self-reports, and the ledger prices `*-cli` providers at $0 because a subscription turn spends the plan, not metered dollars (the physical unit is the plan; its marginal USD is zero). Recording tokens stays honest; pricing them at $0 tells the truth about the money.

Codex is included despite being a policy gray area — OpenAI steers programmatic use toward API keys but doesn't block `codex exec`. `[DEVIATION]`-adjacent: revisit if OpenAI tightens (resurrect-when: `codex exec` starts refusing non-interactive/programmatic runs, or the ToS explicitly forbids wrapping the CLI). The `--dangerously-bypass-approvals-and-sandbox` flag Codex needs (so the MCP bridge subprocess can open the daemon socket) is the widest of the sandbox lifts; a narrower flag may suffice on the live CLI and should be confirmed.

Env-key import is silent by design: it's the user's own key on their own box, and proposing it would add a click to a flow whose whole point is zero-click. It's written to the secrets file (mode 0600) so there stays one source of truth.

Vision-check: no new primitive (the CLI is a swapped-in brain behind the existing loop); transport-agnostic (the ladder is not a human-only path — a spawned child rides the same secret/probe resolution); nothing blocks on humans (a missing backend bounces `chat.input` with a legible reason, and `chat.cli-auth-lost` flips status to needs-login without tearing the loop down). Session resume is in-memory per thread — a daemon restart drops the CLI session id, and the next turn opens a fresh session, re-sending the rendered transcript. When a real API key later arrives, the daemon tears the CLI loop down and re-brings-up in API mode: the CLI was always the fallback, not the choice.

### 2026-07-11 follow-up — security/correctness fixes + known v0 limitations

A review of the just-landed CLI-brain feature found real bugs; fixed surgically (codex prompt now routes through stdin via the documented `codex exec -` / `codex exec resume <id> -` form, closing an argv flag-injection hole where a message starting with `-c mcp_servers.*.command=...` could inject an arbitrary MCP binary while the turn ran `--dangerously-bypass-approvals-and-sandbox`, and simultaneously removing the ~128KB argv E2BIG limit on post-restart concatenated turns; dispatch now rejects unknown `agentId`s instead of treating a missing agent row as unrestricted scope; a failed delegated turn drops the dead CLI session id so the next turn opens fresh; `oneShot` throws on an in-band CLI error instead of returning a fake empty success; stdin gets an EPIPE handler; the mail high-water mark no longer advances on resume turns that never sent the sidebar; the needs-login flag only clears on the root loop's own turn-end). These are wired; the following are **known v0 limitations left in place**, to resurrect when they bite:

- **CLI mode is effectively single-agent.** Child agents spawned in CLI mode are tool-incapable — the manager receives only the `cliBrainToLlm` shim, no `cliBrain`/MCP bridge — so a spawned child cannot reach OLL-E's tools. `spawn_agent` is still advertised, so an agent can create a child that then silently can't act. Fix later by wiring `cliBrain` through the manager or masking delegation tools in CLI mode.
- **Keyless boot blocks on live probes.** Bringup awaits two live "hello" probes (`claude` then `codex`), ~40s worst case, billing the subscription every daemon start; the pid file is written only after bringup, so the probe window widens the pid-file/socket-steal race. Follow-up: cache/persist the last successful brain, or write the pid + go live before probing.
- **No turn timeout/abort into server-side tools; unbounded buffers.** Delegated turns have no timeout or abort propagation to server-side tool execution, and the error-classification `raw` buffer plus the per-thread truncation-state maps are unbounded.
- **Codex `exec resume` usage semantics unverified.** Unknown whether `turn.completed.usage` on a resumed session is per-turn or session-cumulative. If cumulative, resumed turns inflate ledger token counts (still $0, but `olle stats`/token-budgets skew). Needs one live check.
- **Codex sandbox narrowed to `read-only`, unconfirmed on the live CLI.** The `--dangerously-bypass-approvals-and-sandbox` lift is replaced by `-c sandbox_mode="read-only" -c approval_policy="never"` (`SANDBOX_ARGS` in `codex.ts`) — closing the major finding that a misfired or steered turn could touch host state through Codex's own shell outside OLL-E's permission gate. The reasoning that this doesn't break tools: Codex launches MCP servers as its own stdio subprocesses, outside the *shell* sandbox `read-only` restricts, so the `olle mcp-bridge` should still reach the daemon socket. Not verified against a live `codex` — the original wide flag was chosen partly out of doubt here. **Smoke-check when a logged-in codex is available; symptom if wrong: CLI-mode tool calls fail because the bridge can't open the socket. Revert is one line — set `SANDBOX_ARGS` back to `["--dangerously-bypass-approvals-and-sandbox"]`.**
...
## 2026-07-11 — local-llm starter

Tenth starter: adapter for a local OpenAI-compatible server (llama.cpp, vLLM, LM Studio). Two operational tools — `local_llm_generate` (auto-picks the model when exactly one is served, surfaces `reasoning_content` from thinking models, 32KB spill cap) and `local_llm_models`. Two calls worth logging. First: no SSRF guard, deliberately — the web starter guards because tool input is an untrusted URL; here `baseUrl` is operator config aimed at localhost, and guarding would break the extension's entire purpose. The guard belongs to the trust boundary, not to "anything that fetches." Second: this is a tool, not a brain — the chat loop's provider adapters stay core; an agent *uses* local weights for bulk/private generation, it doesn't think on them. Auth is optional physics: `config.apiKeySecret` names a secret sent as Bearer when set (llama.cpp `--api-key` proved this live), unset means no header. Tested e2e against a real llama-server/Qwen3.6-27B on this host, install→smoke→register→generate.

## 2026-07-11 — `slack` starter (Socket Mode base pipe)

Shipped the `slack` base adapter, moving it from the v0.1 deferred list. Socket Mode over the Events-API webhook or legacy RTM: the daemon lives behind NAT with no public HTTPS endpoint, and Socket Mode is the only first-party method that dials *outbound* (`apps.connections.open` → `wss://`); RTM is dead for granular-scope apps and a public Request URL violates the NAT constraint. The one cost — Socket Mode apps can't list in the Marketplace — is irrelevant for a self-hosted daemon. Two tokens, a structural first: an app-level `xapp-` token (`connections:write`) opens the socket, a bot `xoxb-` token makes every Web API call; every other channel starter had one secret. Ring-buffer context, not history polling: Slack downgraded `conversations.history` to Tier-1 (1 req/min, 15 msgs) for non-Marketplace apps in 2025 (enforced for existing installs 2026-03-03), so `slack_fetch_context` is the same in-memory ring buffer honesty as `telegram_fetch_context` — the socket is the only reliable source of what was said. Ack-first-then-process per envelope (~3s deadline) and a generation-counter zero-gap refresh (dial the replacement, let the old socket's close be ignored — Slack tears its side down; overlap dedupes by `event_id`). Staleness on last-inbound-frame is the zombie detector (WHATWG WebSocket exposes no manual ping). Outbound leans on `markdown_text` (server-side CommonMark) with a plain-text fallback, deleting the mrkdwn regex pipeline telegram needed. Live-workspace verification (real handshake, native `chat.startStream` tier, `markdown_text` fidelity) is pending — no Slack credentials on this host. The `slack-communication` behavior bridge (wake-word chat, delivery-audit events, `slack:<channel>:job:<jobId>` standing-job routing) is the deliberate follow-up; the thread-id shapes are reserved.

## 2026-07-11 — Plan: starter versioning and distribution (design only)

Starters are 3,646 lines of string-wrapped source in the binary with no update path — an installed copy and its shipped template share no link, so an upstream bugfix reaches a locally-modified install only by clobbering it, which contradicts the premise that agents clone *and modify*. Studied pi-mono closely (see REFERENCES.md): took its source-string grammar (`repo@ref` is the whole package format), never-implicitly-moved pins, and registry-less discovery; left its pristine-install reconcile (`reset --hard`), because our installed copy and the agent's modifications are the same directory by design. The survey found nobody supports modified-install-plus-upstream-update — every ecosystem overwrites, resets, or forks — because their consumers are humans who don't merge; ours are agents who do. Plan (docs/design/STARTER-DISTRIBUTION.md): `upstream {repo, subdir, ref, commit}` in manifest.json as the install record and merge base; per-file three-way merge via `git merge-file` inside the existing extensions repo; two new core meta-tools riding the unchanged propose→write→smoke→hot-load loop with `revert_extension` as rollback; GitHub monorepo with per-starter tags and a CI-generated index, consumed over git protocol; binary keeps a build-time snapshot of the repo permanently for offline first-boot. ants.land investigated and rejected as substrate — agent-first npm registry by an anonymous solo dev, tarballs not git, 12-hour deletion of unclaimed publishes; watch, don't build on. No code changed; this entry marks the plan landing, implementation is a future session.

## 2026-07-18 — MicroVM isolation: agent code is untrusted by construction

**Prior framing.** v0 listed "no sandbox beyond process boundary" as an explicit non-goal (ROADMAP), and the whole extension loop was built on it: `load(name)` `await import()`s an agent-authored `index.ts` straight into the daemon's module graph, `smoke.ts` runs the same way *before any permission gate*, each declared secret is read off disk as plaintext and injected by closure into `api.secrets`, and "fault isolation" is a circuit breaker that attributes crashes by stack-string path matching (`src/daemon/fault-isolation.ts`). There is no address-space boundary between agent code and the host — a compromised or buggy extension reads every secret the daemon resolved, can exfiltrate anywhere, and can take down the process. The CLI mcp-bridge tool path also lets a caller assert its own `agentId` (`src/mcp/dispatch.ts`), trusting the caller for identity.

**New framing.** The MVP-v0 phase is over (the digest loop earned it); agent-authored code is now **untrusted by construction**. It runs in a per-agent Firecracker microVM with **no network device** — all egress is forced through a host-side credential broker that injects secrets at the edge, so secrets never enter the guest (REST or WebSocket). Smoke tests move into the guest. Trigger sockets/timers move into the guest (or, for WebSockets, into the broker). Guest connections are bound to a VM identity by the per-VM socket directory, so tool calls can't self-assert an agent. Guest-emitted events are `bus.publish`ed with host-stamped identity, never `inject`ed — a guest is a subordinate execution context, not a federation peer.

**Reasoning.** Isolation is agency infrastructure, not feature surface: code that cannot hurt the host can be trusted with less approval friction — the more-agentic path. It is the deterministic environmental boundary that holds when model-layer judgment doesn't, which is exactly the lesson Anthropic published from containing Claude across its own products: "if credentials never enter the sandbox, they can't be exfiltrated, regardless of whether the cause is a user, a model finding a 'creative' path, or an attacker." Their credential *masking* (a per-session sentinel the proxy swaps for the real value only on declared `injectHosts`) is this design's placeholder scheme feature-for-feature — validation that the shape is right. Their tiering also endorses the VM choice: bubblewrap/gVisor for their own trusted-ish code, **full VMs with vsock reserved for Claude Cowork's untrusted-code case** — which is what agent-authored extensions are. And their sharpest warning shapes the build: "our custom allowlist proxy was the piece that failed" while battle-tested hypervisors/seccomp/gVisor held. So the broker (`src/vm/broker.ts`) is the highest-risk component, and Phase 3 opens with a spike of Anthropic's open-source `@anthropic-ai/sandbox-runtime` proxy to decide adopt-vs-build for its allowlist/SSRF/TLS-terminating transport half (the OSS package does *not* implement sentinel substitution — that stayed closed in Claude Code — so the injection layer is ours regardless). One advantage falls out of the no-network-device shape: the broker always makes the upstream TLS call itself, so masking needs no TLS-terminate flag and the domain-fronting hole in a hostname-only allowlist is structurally closed. **Decisions:** per-agent VMs now, with a `placementFor(agentId, manifest)` seam for future pooling by group (revisit at >8 concurrent VMs); broker-owned WebSockets; Linux/KVM Firecracker first, with macOS (vfkit) and a bubblewrap fallback tier designed behind the `VmBackend` interface but deferred until Linux is proven; hosts with no backend fall back to in-process legacy execution flagged by a durable `extension.unisolated` event. The CLI mcp-bridge self-asserted-agentId gap is *not* closed by this work (only guest connections get structural binding) — deferred, recorded here so the asymmetry is on the record. This reverses the "no sandbox beyond process boundary" non-goal from the v0 ROADMAP; see LOG 2026-07-08 (push-first) for the phase-gate reasoning this transition clears.

## 2026-07-16 — Standing jobs: fresh thread per fire by default

A standing job fired onto one stable per-job thread (`cron:<jobId>`, `discord:<channelId>:job:<jobId>`, …), so every fire's turn saw the whole transcript of every prior fire — a prompt that grows without bound and drags stale context (last week's digest sitting in this morning's). That contradicts the reasoning already logged for the misfire policy: a standing job's value is fresh-at-fire-time. New default: each fire opens a *new* thread with no memory of prior fires. `fireJob` mints a per-fire ULID and folds it into the thread id. An agent that genuinely wants the running context opts into `threadMode: "shared"` on `schedule_task`, which restores the old single-thread behavior. Retroactivity without a migration: `parseCronConfig` normalizes a missing/malformed `threadMode` to `"fresh"` at parse time, so every pre-change row (none of which carry the field) becomes fresh on read — no ALTER, no backfill. Thread-id shape: the fire segment sits *before* `:job:<jobId>` on channel ids so the `:job:` segment stays terminal — cloned bridge extensions already live in real users' `~/.olle` and parse the jobId with an end-anchored `/:job:([^:]+)$/`; that regex must keep capturing the jobId. Cli ids carry no such contract so the fire segment appends; the asymmetry is deliberate and commented at the thread-id source. The cloned templates' regexes were left untouched — the whole point of putting the fire segment before `:job:` is that they keep working unmodified. Fresh threads would merely trade unbounded prompt growth for unbounded daemon memory, disk, and sidebar scans, so `fireJob` marks only fresh-fire threads disposable. The agent loop keeps the newest 100 completed disposable threads per agent and deletes older snapshots, including crash-left snapshots at startup. Active, shared, and ordinary threads remain untouched; durable bus events preserve the audit trail.
